...
public final class HttpHeader {
	...
	public static final HttpHeader readHeader(InputStream in) throws IOException {
		...
		while ((c = (char) in.read()) != '\n') {
			builder.append(c);
			if (builder.length() == MAXLINESIZE) {//不接受过长的头部字段  
				break;
			}
		}
		...
		//如能识别出请求方式则继续，不能则退出  
		if (header.addHeaderMethod(builder.toString()) != null) {
			do {
				...
				if (builder.length() > 1 && header.notTooLong()) {//如果头部包含信息过多，抛弃剩下的部分  
					header.addHeaderString(builder.substring(0, builder.length() - 1));
				} else {
					break;
				}
				total.append(builder);
